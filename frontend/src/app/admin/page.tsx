...
import { Users, ShoppingBag, CreditCard, Package } from 'lucide-react';

export default function AdminDashboardPage() {
  const [stats, setStats] = useState<{ users_total?: number; orders_total?: number; orders_paid?: number; products_total?: number } | null>(null);
  ...
  const cards = [
    ...
    { label: '결제 완료', value: stats?.orders_paid ?? '-', icon: CreditCard, color: 'bg-purple-500' },
    { label: '등록 상품', value: stats?.products_total ?? '-', icon: Package, color: 'bg-orange-500' },
  ];
